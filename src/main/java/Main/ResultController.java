...
public class ResultController {
    ...
    private void initialize() {
        correct = EasyController.correct;
        wrong = EasyController.wrong;

        correcttext.setText("Correct Answers : " + correct);
        wrongtext.setText("Incorrect Answers : " + String.valueOf(EasyController.wrong));

        marks.setText(correct + "/10");
        float correctf = (float) EasyController.correct/10;
        correct_progress.setProgress(correctf);

        float wrongf = (float) EasyController.wrong/10;
